...
function Shard(options) {
        ...
        */
        ...
        this._registrarPath =
                '/' +
                (this._shardPath.substring(this._shardPath.indexOf('/', 1) + 1))
                .split('.').reverse().join('/') +
                '/pg';
        ...
        */
        ...
        self._zk = zkplus.createClient(self._zkCfg);
...
Shard.prototype.init = function init() {
        ...
        self._election.on('leader', function() {
                ...
                log.info('clearing previousHeartbeat', intervalId);
        ...
        self._election.on('newLeader', function(leader) {
                ...
                postgresMan.standby(pgUrl, backupUrl, function(err) {
                        ...
                        intervalId = setInterval(function() {
                                heartbeatClient.postHeartbeat(function() {});
                        }, self._heartbeatInterval);

                        log.info('starting heartbeat', intervalId);
                });
        });
...
function _writeRegistrar(self, callback) {
        ...
        zk.creat(self._registrarPath, opts, callback);
}
