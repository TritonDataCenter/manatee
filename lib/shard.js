                                 ...
                                 function(err, listener)
                        {
                                ...
                                listener.on('children', function(children) {
                                        children.forEach(function(child) {
                                                if (child === 'error') {
                                                        log.error('got shard error, going to readonly mode');
                                                        // do some stuff
                                                }
                                        })
                                });
                ...
                function checkForError(_, cb) {
                        zk.get(self._shardErrorPath, function(err, obj) {
                                if (err && err.code === ZK.ZNONODE) {
                                        ...
                                        // do some stuff, i.e. go readonly
                                        log.error('got shard error, ' +
 ...
 */
function _goReadOnly(self) {
        var log = self._log;
        log.warn('entering goReadOnly');
        ...
        self._postgresMgr.primary('', function(){
                setInterval(function(){
                        log.info('In forced read only mode due to error');
                }, 2000);
        });
}
