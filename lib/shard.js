 ...
 * @param {object} options.HeartbeatClientCfg
 * @param {object} options.HeartbeatServerCfg
 * @param {object} options.PostgresMgrCfg
...
function Shard(options) {
    ...
    /** @type {HeartbeatServer} The heartbeat server that listens for standbys */
    ...
    /** @type {object} The sample heartbeat client cfg. //TODO: move hbc creating to a createclietn() call*/
...
function _transformPgUrl(self, url_) {
    return 'tcp://postgres@' + url_.split('-')[0] + ':' +
        self._postgresMgr.getUrl().port + '/postgres';
}

function _transformBackupUrl(self, url_) {
    return  'http://' + url_.split('-')[0] + ':' + self._backupPort;
}

function _transformHeartbeatUrl(self, url_) {
    return  'http://' + url_.split('-')[0]+ ':' + self._heartbeatServerCfg.port;
}
...
function _writeRegistrar(self, callback) {
    ...
    var opts = {
        ...
        object: {
            type: 'database',
            database: {
                primary: self._postgresMgr.getUrl(),
                ttl: self._ttl
            }
        }
    };
