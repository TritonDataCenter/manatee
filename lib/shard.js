 ...
 * writes. As such, we need to check for 2 conditions before we can become the
 * primary.
 * 1) We were not previously an sync.
 * 2) We can replicate correctly to a synchronous peer.
 */
    ...
    var tasks = [
        ...
        function checkWasAsync(_, cb) {
            self._postgresMgr.canBeMaster(function(can) {
                ...
                // to master.
                if (!can) {
                    log.info({can: can}, 'Shard._leader: was async, aborting ' +
                        '_leader and waiting for other peers');
        ...
        vasync.pipeline({funcs:  [
            ...
            function _waitForAnotherNode(_, cb) {
                ...
                self._zk.watch(self._shardElectionPath, {method: 'list'},
                    ...
                    // stop waiting.
                    self._zk.readdir(self._shardElectionPath, function(err, nodes) {
                        log.trace({err: err, nodes: nodes},
...
function _waitOrStart(self, cb) {
    ...
    vasync.pipeline({funcs: [
        ...
        function _waitForAnotherNode(_, _cb) {
            cb = once(cb);
            if (!_.can) {
                log.info('Shard._waitForStart: was async, waiting for other nodes to join');
