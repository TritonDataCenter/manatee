...
function joinShard(self) {
      ...
      determineRole(self, peers, function(err) {
        ...
        if (self.role === ROLE.PRIMARY) { // leader
          ...
          watchRegistrar(self, regWatch, false, function(err, shardInfo) {
            ...
            if (!shardInfo) {
              log.warn('primary dne after election, re-electing via restart');
              process.exit(0);
            }
   ...
   *
   * If the current role is STANDBY:
   * 1) check that the new registrar exists. If DNE, we set a watch on the
   * parent path of the registrar and wait. This implies that there could be
   * deadlocks if the new primary fails. But then we are down to 1 peer and
   * operator intervention is needed anyway.
  ...
  function regWatch(type, state, path) {
    var log = self.log;
    log.info('reg watch fired type %s, state %s, path %s', type, state, path);
    // Get the registrar and add the rewatch
    watchRegistrar(self, regWatch, false, function(err, shardInfo) {
      if (err) {
        return self.emit('error', err);
      }
      ...
      if (self.role === ROLE.STANDBY) {
        switch(type) {
          ...
          case ZooKeeper.ZOO_CHILD_EVENT:
            //TODO: What to do when shardInfo dne? wait? restart?
            if (!shardInfo) {
          ...
          case ZooKeeper.ZOO_DELETED_EVENT:
            ...
            if (prevSync && (prevSync === self.url)) {
              ...
              // if prevSync dne, this means the primary did exist but died
              // before we were able to get its registrar info. we re-elect
              // by restarting manatee
              log.warn('primary dne, re-electing via restart');
              process.exit(0);
            }
            break;
          default:
...
function watchRegistrar(self, watch, errOnNotExist, callback) {
  var log = self.log;
  log.info('getting registrar as standby', self.registrarPath);
                      ...
                      function(rc, msg, stat, data)
  {
    ...
    if (rc === ZooKeeper.ZNONODE && !errOnNotExist) {
      ...
      getPeers(self, self.registrarPathPrefix, watch, function(err, shards) {
        ...
        log.warn('no registrar found under registrar parent path');
        return callback();
      });
    } else if (rc !== 0) { // there is an error getting the registrar
      var err = {
        rc: rc,
        msg: msg,
        stat: stat,
        myPath: self.myPath,
        registrarPath: self.registrarPath
      };
      log.error({err: err}, 'unable to get shard registrar info');
      return callback(err);
    } else { // mainline, got the registrar
