...
function _newStandby(ctx, cb) {
    ...
    if (!self._election.amLeader) {
        log.info({
            currStandby: self._currStandby
        }, 'not primary, ignoring standby heartbeats,but updating currStandby');
...
function _initAndCleanupZkNodes(self, callback) {
    ...
    vasync.pipeline({funcs: tasks, arg: {}}, function(err) {
        if (err) {
            log.error('Unable to initialize zk paths or delete previous emphemeral nodes');
            return callback(err);
        } else {
            log.info('finished initializing zk paths and deleting previous emphemeral nodes');
            return callback();
        }
    });
}
