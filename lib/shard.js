 ...
 * @param {number} options.backupPort
 * @param {number} options.postgresPort
...
function Shard(options) {
    ...
    /** @type {number} The port used by the pg backup service */ //TODO: remove this
...
function _transformHeartbeatUrl(self, _url) {
    return  'http://' + _url.split('-')[0]+ ':' + self._heartbeatServerCfg.port;
}
...
function _writeRegistrar(self, callback) {
    ...
    var opts = {
        ...
        object: {
            type: 'database',
            database: {
                primary: _transformPgUrl(self, self._ip),
                ttl: self._ttl
            }
        }
    };
    ...
    log.info({
        ...
        url: _transformPgUrl(self, self._ip),
        opts: opts
