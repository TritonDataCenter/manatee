...
/**
 * Writes the host urls of this shard such that clients can discover them
 * @param {function} callback The callback of the form f(rc, msg, stat).
 */
//Shard.prototype.writeRegistrar = function writeRegistrar(callback) {
  //var self = this;
  //var log = self.log;
  //log.debug('entering writeregistrar');
  //var hosts = {
    //primary: self.primary.url,
    //sync: self.sync ? self.sync.url : null,
    //async: self.async ? self.async.url : null
  //};
  ////TODO: need to set the version
  //this.session.a_set(self.path, JSON.stringify(hosts), -1, callback);
//};
