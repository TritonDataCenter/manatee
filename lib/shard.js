...
function _waitForPrimary(ctx, callback) {
    var self = ctx.self;
    var log = self._log;
    var zk = self._zk;

    log.info('Shard_.waitForPrimary: entering');
    ...
    zk.once('close', function() {
        vasync.pipeline({funcs:  [
            ...
            function _initAndCleanup(_, _cb) {
                _initAndCleanupZkNodes(self, _cb);
            },
            ...
            function _joinElection(_, cb) {
                ...
                // resume the queue after we've rejoined to listen for events
                process.nextTick(self._transitionQueue.resume());

                return cb();
            }
        ]}, function(err) {
            log.info('Shard_.waitForPrimary: exiting');
            // MANATEE-127 resume the queue once this is complete so we can
...
function _init(self, callback) {
    ...
    vasync.pipeline({
        funcs: [
            ...
            function _initAndCleanup(_, _cb) {
                _zkInitAndClean(self, function(err) {
                    if (err) {
                        err = new verror.VError(err);
                    }
                    return _cb(err);
 ...
 */
function _zkInitAndClean(self, callback) {
    var log = self._log;
    var zk = self._zk;
