 ...
 * @param {number} options.postgresPort  The port of the PG instasnce.
...
function Shard(options) {
    ...
    /** @type {string} The backup server port */
    this._backupPort = options.backupPort;

    /** @type {string} The postgresql port */
    this._postgresPort = options.postgresPort;

    /** @type {string} The heartbeat server port */
...
Shard.prototype.TRANSITION_FUNCS = {
    ...
    NEW_STANDBY: function _newStandby(ctx, callback) {
        ...
        var standbyUrl = self._transformPgUrl(standby);
    ...
    EXPIRED: function (ctx, callback) {
        ...
        var log = self._log;
        var expiredStandbyUrl = self._transformPgUrl(expiredStandby);
