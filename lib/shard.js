 ...
 * writes. As such, we need to check for these conditions before we can become
 * the primary.
 *
 ...
 * until another node enters the shard.
 */
function _leader(ctx, callback) {
    ...
    var tasks = [
        ...
        function canBeMaster(_, cb) {
            _canBeMaster(self, function (err, can) {
                ...
                if (!can) {
                    ...
                    self._postgresMgr.stopSyncChecker(false, function (err) {
        ...
        //// MANATEE-127 Check if we used to be an async
        //function checkWasAsync(_, cb) {
            //self._postgresMgr.canBeMaster(function (can) {
                //// if we were an async, then wait for others and do not proceed
                //// to master.
                //if (!can) {
                    //// first stop checking sync state, but ignore errors
                    //self._postgresMgr.stopSyncChecker(false, function (err) {
            ...
            //});
        //},
        //// MANATEE-127 check the case for sync and primary
        //function checkIfLastPrimary(_, cb) {
            //return cb();
        //},
...
/**
 * When a transition to leader occurs -- and we were previously an
 * asynchronous peer, we _must_ not become the leader and wait for another node
 * to enter the shard first. Since an asynchronous peer's data will never be
 * completely up to date, if it becomes the leader, other peers in the shard
 * who used to be the sync or primary will never be able to slave from the
 * async. This in the best of cases, will cause the shard to go into the r/o
 * error state. We want to avoid this, and so if our previous state was that of
 * an async, then we wait for another node to join before we try and join the
 * shard.
 *
 ...
 * this event will always run on the queue when "leader" event is executed and
 * we were preeviously an async. All previously events on the queue can execute
 * safely since if and when a "leader" event is executed, it will call into
 * this function. Once this function is running and on the queue, we do the
 * following:
 *
...
function _waitForOtherPeers(ctx, callback) {
    ...
    // need to unregister from election
    self._zk.close();
...
function _canBeMaster(self, cb) {
    var log = self._log;
    log.info('Shard.canBeMaster: entering');
    cb = once(cb);
    ...
    function getLastPrimary(_cb) {
        ...
        _getShardHistory(self, function (err, history) {
            ...
            // for the last primary.
            // history looks like:
            /* JSSTYLED */
            // {"time":"1382288491791","ip":"10.77.77.23","action":"NewStandby","role":"Leader","master":"","slave":"10.77.77.24","zkSeq":"0000000388"}
        ...
        function _checkRole(_, _cb) {
            ...
            if (!_.role || !_.role.role) {
                ...
                getLastPrimary(function (err, primary) {
                    ...
                    /*
                     * if there was never a primary, then it's safe to be a
                     * primary.
                     */
                    if (!primary) {
                        canBeMaster = true;
                     ...
                     */
                    var myLeaderIp = url.parse(_.role.leader).hostname;
                    if (primary.ip === myLeaderIp) {
                        canBeMaster = true;
                        return _cb();
                    } else {
                        canBeMaster = false;
