...
function Shard(options) {
  ...
  assertString('options.shardPath', options.shardPath);
  assertString('options.shardId', options.shardId);
  assertString('options.registrarPath', options.registrarPath);
  assertString('options.xlogPath', options.xlogPath);
  assertString('options.xlogPathPrefix', options.xlogPathPrefix);
  assertString('options.registrarPathPrefix', options.registrarPathPrefix);
  ...
  /**
   * The path where the current primary xlog location is stored.
   */
  this.xlogPath = options.xlogPath;
  this.xlogPathPrefix = options.xlogPathPrefix;
...
Shard.prototype.init = function init() {
  ...
  var client = zkplus.createClient({
    servers: [{
      host: zkCfg.connect.split(';')[0],
      port: zkCfg.connect.split(';')[1]
    }]
  });

  client.on('connect', function () {
    log.info('mkdir -p shard and registrar paths');
    client.mkdirp(self.registrarPath, function (err) {
...
/**
 * Watches the xlog position of the master in the shard.
 * This only executes if ROLE = STANDBY
 * emits 'xlog' event if the xlog changes.
 * @param
 */
Shard.prototype.watchXlog = function watchXlog(callback) {
  var self = this;
  var log = self.log;
  if (self.role === ROLE.PRIMARY) {
    log.warn('role is not standby, exiting watchXlog');
    return callback();
  }
  log.info('watching xlog', self.xlogPath);

  /**
   * The algorithm is as follows:
   * 1 First try and get the znode.
   * 2 If it DNE, then set a watch on its parent
   * 3 If the parent watch fires, try and get the Znode again.
   * 4 If the znode exists, set watch on znode. else go to step 2
   */

  self.session.aw_get(self.xlogPath, xlogWatch,
                      function(rc, msg, stat, data)
  {
    // set a watch even if the znode dne
    if (rc === ZooKeeper.ZNONODE) {
      log.info('xlogPath DNE, watching parent', self.registrarPathPrefix);
      // if the xLog doesn't exist, watch the parent prefix for changes
      getPeers(self, self.xlogPathPrefix, xlogWatch, function(err, shards) {
        if (err) {
          log.error('error while trying to get parent xlog znode');
          return self.emit('error', err);
        }
        shards.forEach(function(path) {
          log.debug('got shards', path);
          if (path === self.xlogPath) {
            log.debug('got my xlog', path);
            // rewatch to get the data
            return self.watchXlog(callback);
          }
        });
      });
      return callback();
    } else if (rc != 0) { // if some other err
      var err = {
        rc: rc,
        msg: msg,
        stat: stat,
      ...
      log.error('unable to get xlog path prefix', err);
      return callback(err);
    } else { // else the xlog pos exists
      log.debug('got xlog pos', data);
      self.emit('xlog', JSON.parse(data));
      return callback(null, JSON.parse(data));
    }
  });

  function xlogWatch(type, state, path) {
    var log = self.log;
    log.info('watch fired, xlog possibly changed!', type, state, path);
    // rewatch
    self.watchXlog(callback);
  }
};

/**
 * Writes the current xlog location
 * @param {string} xlog The xlog location
 * @param {function} callback The callback of the form f(err)
 */
Shard.prototype.writeXlog = function writeXlog(xlog, callback) {
  var self = this;
  var log = self.log;
  log.info('writing xlog position to zookeeper', xlog);

  // first try to set the znode
  self.session.a_set(self.xlogPath, xlog,
                     -1, function(rc, msg, path)
  {
    // if znode dne, create the znode
    if (rc === ZooKeeper.ZNONODE) {
      log.info('previous registrar DNE, writing new registrar znode');
      self.session.a_create(self.xlogPath, xlog, ZooKeeper.ZOO_EPHEMERAL,
        function(rc, msg, path)
      {
        if (rc !== 0) {
          ...
          log.error('unable to write xlog', err);
          self.emit('error', err);
          return callback(err);
        }

        log.debug('successfully written xlog', path);
        return callback(null);
      });
    } else if (rc !== 0) {
      var err = {
      ...
      log.debug('successfully updated xlog', path);
      return callback(null);
    }
  });
};
 ...
 * in stdby state
 * Upon initialization, we try and join the shard
 */
function joinShard(self) {
              ...
              function(err, path)
  {
    ...
    // don't set the watch while fetching peers for the first time.
    // the watch gets set after role determination is made.
  ...
  function regWatch(type, state, path) {
    ...
    watchRegistrar(self, regWatch, false, function(err, shardInfo) {
      ...
      if (shardInfo) {
        self.shardInfo = shardInfo.database;
      }
