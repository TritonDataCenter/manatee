 ...
 * @param {number} options.heartbeatServerPort The port of the heartbeat server.
...
function Shard(options) {
    ...
    assert.number(options.heartbeatServerPort, 'options.heartbeatServerPort');
    ...
    /** @type {number} The heartbeat server port */
    this._heartbeatServerPort = options.heartbeatServerPort;
    ...
    //TODO: move hbc creation to a createclient() call
    ...
    /**
     * @type {string}
     * Serialized version of this shard's services. This colon seperated into 4
     * fields.
     * f1: IP Address
     * f2: PG port
     * f3: backup port
     * f4: heartbeatServer port
     */
    this._electionPrefix = self._ip + ':' + self._postgresPort + ':' +
        self._backupPort + ':' + self._heartbeatServerPort;
...
Shard.prototype.TRANSITION_FUNCS = {
    ...
    LEADER: function _leader(ctx, callback) {
        ...
        vasync.pipeline({funcs: [
            ...
            function canBeLeader(_, cb) {
                self._canBeMaster(function (err, can) {
                    if (err) {
    ...
    NEW_LEADER: function _newLeader(ctx, callback) {
        ...
        var heartbeatUrl = self._transformHeartbeatUrl(myLeader);
        self._leader = pgUrl;
        var postgresMgr = self._postgresMgr;
        log.info({
            leader: myLeader,
        ...
        vasync.pipeline({funcs: [
            ...
            function stopHeartBeat(_, cb) {
                if (self._heartbeatClient) {
                    log.info('Shard.NEW_LEADER: stopping heartbeat');
                    self._heartbeatClient.stop();
                }
            ...
            function startHeartbeat(_, cb) {
                log.info({
                    ...
                    ip: self._ip
                }, 'Shard.NEW_LEADER: starting heartbeat');
    ...
    /**
     * A new standby event signifies that we may have a new standby -- this is
     ...
     * @param {Shard-cb} callback
     */
    NEW_STANDBY: function (ctx, callback) {
        var self = ctx.self;
        var log = self._log;
        var postgresMgr = self._postgresMgr;
        var standbyUrl = ctx.standby;
        ...
        vasync.pipeline({funcs: [
            function checkStandby(_, cb) {
                ...
                }, 'Shard.NEW_STANDBY: checking standby status');

                if (standbyUrl === self._currStandby) {
                    log.info('Shard.NEW_STANDBY: standby not changed');
                    _.skipPg = true;
                    _.skipHistory = true;
                }
                ...
                if (!self._election.amLeader) {
                    log.info({
                        currStandby: self._currStandby
                    }, 'Shard.NEW_STANDBY: not primary, ignoring standby ' +
            ...
            if (err && err.__walCorruption) {
                ...
                self._writeErrorNode(self._currStandby, function (err2) {
                    ...
                    log.info('Shard.NEW_STANDBY: entering shardError state');
                    self._safeMode();
                    // don't return cb to stop any further state changes
                    return (undefined);
                });
     ...
     * standby is the current standby, and we are the leader of the shard,
     * then we need to explicitly remove the standby from postgresql.conf, as
     * it was slaving synchronously. If we are not the leader, or the standby
     * isn't the current standby, there is nothing to do since in the former
     * case, the standby was an async and no config changes are required, and
     * in the latter, the standby isn't current, so regardless of whether we
     * are the leader or not, the standby isn't in the configs.
     *
     * We always write the history node if the standby is the current standby
     * for posterity.
     *
     * @param {object} ctx The context associated with this transition.
     * @param {object} ctx.self The shard object.
     ...
     * @param {Shard-cb} callback
     */
    EXPIRED: function (ctx, callback) {
        var self = ctx.self;
        var postgresMgr = self._postgresMgr;
        var expiredStandby = ctx.expiredStandby;
        var log = self._log;
        var expiredStandbyUrl = expiredStandby;
        ...
        vasync.pipeline({funcs: [
            function checkStandbyAndIsLeader(_, cb) {
                ...
                if (expiredStandbyUrl === currStandby &&
                    self._election.amLeader)
                {
                    log.info({
                        currStandby: currStandby,
                        expiredStandby: expiredStandbyUrl,
                        isLeader: self._election.amLeader
                    }, 'Shard.EXPIRED: expired standby matches currStandby ' +
                       'and isLeader, removing expired standby');
            ...
            function writeHistory(_, cb) {
                if (expiredStandbyUrl === currStandby) {
                    ...
                    // don't write history if a non-current standby expires,
                    // there's no point.
                    log.info({
                        currStandby: currStandby,
                        expiredStandby: expiredStandbyUrl,
                        isLeader: self._election.amLeader
                    }, 'Shard.EXPIRED: not writing history, standby isn\'t ' +
                    'current');
                return cb();
                }
 ...
 * @param {Shard-cb} callback
 */
Shard.prototype.shutdown = function (callback) {
    var self = this;
    var log = self._log;

    log.info('Shard.shutdown: entering');

    vasync.parallel({funcs: [
        function _stopPostgres(cb) {
            self._postgresMgr.shutdown(cb);
        },
        function _stopZookeeper(cb) {
            self._zk.on('close', function () {
                log.info('Shard.shutdown: zk closed.');
                return cb();
            });
            self._zk.close();
        }
    ...
    self._zk.once('close', function () {
        vasync.pipeline({funcs:  [
            function _createZk(_, _cb) {
                _cb = once(_cb);
                self._createZkClient(self._zkCfg, function (err, zkClient) {
                    if (err) {
 ...
 */
Shard.prototype._canBeMaster = function (cb) {
    var self = this;
    var log = self._log;
            ...
            if (!_.role || !_.role.role) {
                ...
                getLastPrimary(function (err, primary) {
                    ...
                    if (_.role.role === 'primary') {
                        myIp = self._ip + ':' + self._postgresPort;
                    } else {
                        myIp = url.parse(_.role.leader).host;
                    }
                    if (primary.ip === myIp ||
                        /*
                         * MANATEE-186
 ...
 */
Shard.prototype._createZkClient = function (opts, cb) {
...
Shard.prototype._writeErrorNode = function (standby, callback) {
    ...
    zk.creat(self._safeModePath, opts, function (err) {
        return callback(err);
    });
};
...
/**
 * Check and watch the error node. If the error znode exists, put the nodein
 * safe mode.
 *
 * @param {Shard-cb} callback
 */
Shard.prototype._checkAndWatchError = function (callback) {
    ...
    vasync.pipeline({funcs: [
                     ...
                     function (err, listener)
            {
                if (err) {
                    return callback(new verror.VError(err,
                        'unable to set watch on error znode'));
                }
        ...
        function checkForError(_, cb) {
            zk.get(self._safeModePath, function (err, obj) {
                if (err && err.code === zkplus.ZNONODE) {
                    return cb();
                } else if (err) {
                    return cb(new verror.VError(err,
                        'unable to get error znode'));
                } else if (obj) {
                    log.fatal('got error znode, going to safe mode');
                    self._safeMode();
        ...
        if (err) {
            return callback(new verror.VError(
                err, 'unable to check for safe mode state'));
        } else {
    ...
    vasync.pipeline({funcs: [
        ...
        function determineNodes(_, cb) {
            ...
            for (var i = 0; i < _.nodes.length; i++) {
                var node = _.nodes[i];
                // node looks like ip:pgport:bport:hbport-zkseq, so delete
 ...
 * 10.77.77.9:pgPort:backupPort:hbPort-0000000057
 *
 * @return {string} The transformed heartbeat server url, e.g.
 * http://10.0.0.0:5432
 */
Shard.prototype._transformHeartbeatUrl = function (zkNode) {
    var data = zkNode.split('-')[0].split(':');
    return 'http://' + data[0] + ':' + data[3];
};
