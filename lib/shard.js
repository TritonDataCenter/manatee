 ...
 */
SyncStateChecker.prototype.start = function start(primaryUrl, myIp, cb) {
    var self = this;
    var log = self._log;
    log.info({
    ...
    function checkWriteSyncState() {
        var queryString = sprintf(SYNC_STATE_PG_QUERY, myIp);
...
SyncStateChecker.prototype.stop = function stop() {
    var self = this;
    self._log.info({
        primaryUrl: primaryUrl,
        currPrimaryUrl: self._primaryUrl
    ...
    vasync.pipeline({
        funcs: [
            ...
            function deleteCookie(_, cb) {
                fs.unlink(self._cookieLocation, cb);
            }
        ]
    }, function(err, results) {
        return cb(err);
