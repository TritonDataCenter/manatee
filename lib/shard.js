 ...
 * for another node to enter the shard first. Since an this peer's data can't
 * be guaranteed to be up to date.  If it becomes the leader, other peers in
 * the shard who used to be the sync or primary will never be able to slave
 * from the async. This in the best of cases, will cause the shard to go into
 * the r/o error state. We want to avoid this, and so if our previous state was
 * that of an async, then we wait for another node to join before we try and
 * join the shard.
 *
