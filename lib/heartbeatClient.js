...
function _postHeartbeat(self, callback) {
    ...
    self.log.trace({opts: opts}, 'sending heartbeat request');

    self.client.post(opts, function (connectErr, req) {
        self.log.trace({err: connectErr, req: req}, 'returned from connect');
        if (connectErr) {
        ...
        // MANATEE-83 set timeout incase requests get black-holed.
        var timeoutId = setTimeout(function() {
            log.trace({req: req}, 'request timed out');
            req.abort();
            return callback(new Error('request timed out'));
        }, self.requestTimeout);

        req.write('host=' + self.url);
        req.end();
        ...
        req.on('result', function (err, res) {
            ...
            if (err) {
                self.log.warn({err: err}, 'unable to post request to leader');
                return callback(err);
            }
            res.body = '';
