...
function PostgresMan(options) {
  ...
  log.trace('new postgres man with options', options);
}
  ...
  function xlogReceiveLocation(callback)
{
  var self = this;
  var log = self.log;
  log.trace('pgsql received xlog check');
...
PostgresMan.prototype.getTrackingTime = function getTrackingTime(callback) {
  var log = self.log;
  log.trace('querying latest tracking table time');
...
{
  var log = self.log;
  log.trace('create replication tracking table');
