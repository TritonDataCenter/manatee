...
PostgresMan.prototype.health = function health(callback) {
  ...
  pg.connect(conString, function(err, client) {
    ...
    client.query("SELECT NOW() as when", function(err, result) {
      log.info("Row count: %d",result.rows.length);  // 1
      log.info("Current year: %d", result.rows[0].when.getYear());
      return callback(err);
    });
  });
};
...
PostgresMan.prototype.xlogLocation = function xlogLocation(callback) {
  ...
  pg.connect(self.url, function(err, client) {
    ...
    client.query('select pg_current_xlog_location()', function(err, result) {
      log.debug('result', result);
      if (result) {
        var location = result.rows[0].pg_current_xlog_location;
      }
