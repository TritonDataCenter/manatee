...
function status(opts, cb) {
    ...
    if (opts.shard) {
        shardPath = '/manatee/' + opts.shard + '/election';
    }
        ...
        function _getShards(_, _cb) {
            ...
            if (opts.shard) {
                zkClient.getChildren(shardPath, function (err, c) {
                    if (err) {
                        if (err.code === zk.Exception.NO_NODE) {
                            return _cb(
                                new verror.VError(err, 'shard ' +
                                                  opts.shard +
                                                  ' does not exist'));
                        }
                        return _cb(err);
                    }
                    c.sort(compareNodeNames);
                    _.shards[opts.shard] = c;
                    return _cb();
                });
            } else  {
                zkClient.getChildren('/manatee', function (err, c) {
                    if (err) {
                        return _cb(err);
                    }
 ...
 * @param {String} opts.shard The name of the shard.
 * @param {Object} opts.zk The zk client config.
 * @param {Object} opts.zfs The zfs client config.
 */
function rebuild(opts, cb) {
    ...
    vasync.pipeline({ funcs: [
        ...
        function _verify(_, _cb) {
            ...
            var waitCount = 0;
            process.stderr.write('verifying shard status  ');
            checkShard();
            function checkShard() {
                status({shard: shard, zk: config.zkCfg.connStr},
                       function (err, stat) {
        ...
        function _rebuild(_, _cb) {
            ...
            vasync.pipeline({funcs: [
                ...
                function _deleteDataDir(__, _cb2) {
                    console.error('removing data dir');
        ...
        function _getShardStatus(_, _cb) {
            ...
            status({shard: shard, zk: config.zkCfg.connStr},
                ...
                if (stat && stat.primary) {
                    ...
                    console.error('disable all other peers first');
