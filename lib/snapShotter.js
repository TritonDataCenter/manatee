...
function SnapShotter(options) {
    ...
    this._snapshotNumber = options.snapshotNumber;
...
SnapShotter.prototype.start = function start(callback) {
    ...
    setInterval(function () {
        ...
        if (snapshots.length > self._snapshotNumber) {
            ...
            for (var i = 0; i < snapshots.length - self._snapshotNumber; i++) {
                var delSnapshot =
                self._dataset + '@' + snapshots[i];
...
SnapShotter.prototype._pgStopBackup = function (callback) {
    var self = this;
    self._log.info('stopping pg_start_backup');
...
SnapShotter.prototype._pgStartBackup = function (callback) {
    var self = this;
    self._log.info('starting pg_start_backup');
...
SnapShotter.prototype._queryDb = function (query, callback) {
    ...
    var client = new Client(self._pgUrl);
