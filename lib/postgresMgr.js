...
PostgresMgr.prototype.initDb = function initDb(self, callback) {
        ...
        if (shelljs.ls(self.dataDir).length === 0) {
                var err2 = new verror.VError('postgres datadir %s dne',
                                              self.dataDir);
        ...
        postgres.on('exit', function(code) {
                ...
                if (code !== 0) {
                        ...
                        log.info({
                                err: err,
                                dataDir: self.dataDir
                        },'PostgresMgr.initDb: unable to initDb postgres');
                }
                log.info({
                        dataDir: self.dataDir,
                        hbaConf: self.hbaConf
                }, 'PostgresMgr.initDb: copying pg_hba.conf to data dir');
