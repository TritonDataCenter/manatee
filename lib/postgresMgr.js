 ...
 * healthchk fails
 */
function startHealthCheck(self, callback) {
    var log = self.log;
    ...
    if (self.healthChkIntervalId) {
        ...
        self.healthChkIntervalId = setInterval(function () {
            ...
            clozure.timeoutId = setTimeout(function () {
                ...
                /**
                 * Error event, emitted when there's an unrecoverable error with the PostgreSQL instance.
...
function stopHealthCheck(self, callback) {
    var log = self.log;
...
function restart(self, callback) {
    ...
    health(self, function (err) {
        if (err) {
            ...
            return start(self, callback);
        } else {
            log.info('Postgresman.restart: db is running');
            stop(self, function (err2) {
                if (err2) {
                    return callback(err2);
                } else {
                    return start(self, callback);
 ...
 * @param {function} cb The callback of the form f(err).
 */
