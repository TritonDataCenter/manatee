...
PostgresMgr.prototype.primary = function primary(stdby, callback) {
        ...
        var tasks = [
                ...
                function _updateConfigs(_, cb) {
                        updateStandbySetSyncCommit(self, null, cb);
                },
                ...
                function _updateStandby(_, cb) {
                        if (stdby) {
                                updateStandbySetSyncCommit(self, stdby, cb);
                        } else {
                                cb();
                        }
                },
 ...
 */
function updateStandbySetSyncCommit(self, stdby, cb) {
        var log = self.log;
        log.info({
                standby: stdby,
                postgresConf: self.postgresConf
