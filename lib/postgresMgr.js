                ...
                if (err) {
                        ...
                        if (timeElapsed > self.healthChkTimeout) {
                                log.error({
                                        ...
                                        timeElapsed: timeElapsed,
                                        timeOut: self.healthChkTimeout
                                }, 'PostgresMgr.healthChk: health ' +
                                        'check timed out');
                                self.emit('error', verror.VError(err));
                        }
                } else {
                        self.lastHealthChkTime = Date.now();
                }
...
function queryDb(self, query, callback) {
        ...
        client.on('error', function(err) {
                log.warn({err: err}, 'got error while querying db');
                // ignore errors here, since the pgclient will sometimes throw
                // when we shut down postgres
        });
...
function health(self, callback) {
        ...
        try {
                queryDb(self, 'select now() as when', function(err) {
                        if (err) {
                                log.debug({err: err}, 'Postgresman.health: ' +
                                         'failed');
                        }
                        return callback(err);
                });
