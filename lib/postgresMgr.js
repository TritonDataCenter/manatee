...
PostgresMgr.prototype.start = function start(self, cb) {
        ...
        postgres.on('exit', function(code, signal) {
                ...
                if (code !== 0) {
                         ...
                         * If this callback is invoked, it manifests itself by
                         * causing vasync to throw a pipeline error. What we
                         * really want is to indicate this as fatal and exit
                         * manatee.
                         */
                        if (calledBack) {
                                log.fatal('postgres exited unexpectedly, ' +
                                          'exiting manatee, check for pg ' +
                                          'core dumps');
                                self.emit('error', err);
                        }
