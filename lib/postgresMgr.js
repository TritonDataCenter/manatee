...
PostgresMgr.prototype.readOnly = function readOnly(callback) {
    ...
    var tasks = [
        ...
        function _deleteRecoveryConf(_, cb) {
            self._deleteFile(self.recoveryConfPath, function () {
                return cb();
            });
        },
 ...
 */
PostgresMgr.prototype._deleteFile = function (path, callback) {
    var self = this;
    var log = self.log;
...
PostgresMgr.prototype._updatePgConf = function (options, cb) {
    var self = this;
    self._updateConf(options, self.postgresConf, self.postgresConfPath, cb);
}
...
PostgresMgr.prototype._updateRecoveryConf = function (options, cb) {
    var self = this;
    self._updateConf(options, self.recoveryConf, self.recoveryConfPath, cb);
}
 ...
 */
PostgresMgr.prototype._checkRepl = function (cb) {
    var self = this;
    var log = self.log;
    var replReplayLoc = null;
    var replStartTime = Date.now();
    var intervalId = setInterval(function () {
        self._checkReplStatus(intervalId, function (err, _stop, replayLoc) {
            if (err) {
...
PostgresMgr.prototype._checkReplStatus = function (intervalId, callback) {
    var self = this;
    var log = self.log;
    ...
    self._queryDb('select  * from pg_stat_replication', function (err, result) {
