...
PostgresMgr.prototype.standby = function standby(primUrl, backupUrl, callback) {
    ...
    function restore(cb) {
        ...
        zfsClient.restore(function(err2, snapshot) {
            ...
            var cmd = 'pfexec chown -R ' + self.dbUser + ' ' +
                self.dataDir;
            log.info({
                cmd: cmd
            }, 'PostgresMgr.standby: finished backup,' +
            ' chowning datadir');
        exec(cmd, cb);
        return (undefined);
        });
...
PostgresMgr.prototype.stop = function stop(callback) {
    ...
    if (!postgres) {
        var msg = 'postgres handle DNE, was pg started by another process?';
...
/**
 * Update the owner of the the pg data dir back to the postgres user.  ZFS
 * receive operations resets the dir back to root ownership. pg does not allow
 * root ownership on a pg data dir.
 */
