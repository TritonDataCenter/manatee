...
PostgresMgr.prototype.start = function start(self, cb) {
    ...
    postgres.on('exit', function(code, signal) {
        if (code !== 0) {
             ...
             * result of pg core dump.  Since postgres is started as a child
             ...
             * invoked, it manifests itself by causing vasync to throw a
             * pipeline error.  What we really want is to indicate this as
             * fatal and exit manatee.
             */
            if (calledBack) {
...
SyncStateChecker.prototype.start = function start(primaryUrl, myUrl, cb) {
    ...
    function createPgClient(url, _cb) {
        //if (!self._pgClient) {
        // TODO: don't create a new client everytime
        // hack to just create a new client everytime for now, there could be
        // nasty race conditions due to the interval still running.
        self._pgClient = new pg.Client(url);

        self._pgClient.connect(function(err) {
            // try again next time
            if (err) {
                //log.warn({err: err, url: url}, 'unable to connect to pg');
                self._pgClient.end();
                self._pgClient = null;
    ...
    function checkWriteSyncState() {
        ...
        createPgClient(self._primaryUrl, function(err) {
            if (err) {
                //log.warn({err: err}, 'unable to connect to pg, ' +
...
SyncStateChecker.prototype.stop = function stop(cb) {
    var self = this;
    self._log.info({
        currPrimaryUrl: self._primaryUrl
    ...
    vasync.pipeline({
        funcs: [
            ...
            function deleteCookie(_, _cb) {
                fs.unlink(self._cookieLocation, _cb);
            }
        ]
    }, function(err, results) {
