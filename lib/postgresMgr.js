...
SyncStateChecker.prototype.start = function start(primaryUrl, myUrl, cb) {
    ...
    function checkWriteSyncState() {
        ...
        log.debug({queryString: queryString}, 'querying sync state from pg');
        self._pgClient.query(queryString, function(err, result) {
            if (err) {
                ...
                if (result.rows[0] && result.rows[0]['sync_state']) {
                    ...
                    fs.writeFile(self._cookieLocation, state, 'utf8', function(err)
                    {
                        if (err) {
                            log.warn({
                                err: err,
    ...
    vasync.pipeline({
        funcs: [
            ...
            function createNewPgClient(_, _cb) {
                self._pgClient = new pg.Client(primaryUrl);
                self._pgClient.connect(function(err) {
                    if (err) {
                        err = new verror.VError(
                            err,
                            'SyncStateChecker.start: unable to connect to pg'
                        );
                    }
                    return _cb(err);
                });
