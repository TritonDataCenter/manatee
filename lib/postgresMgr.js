 ...
 *
 * @return {PostgresMgr} Instance of PostgresMgr.
 */
function PostgresMgr(options) {
    ...
    this.log = options.log.child({component: 'PostgresMgr'}, true);
    var log = this.log;
    var self = this;
...
PostgresMgr.prototype.readOnly = function readOnly(callback) {
    ...
    var tasks = [
        ...
        function _deleteRecoveryConf(_, cb) {
            self.deleteFile(self.recoveryConfPath, function () {
                return cb();
            });
        },
...
PostgresMgr.prototype.standby = function standby(primUrl, backupUrl, callback) {
    ...
    var tasks = [
        ...
        function _setSyncCommitOff(_, cb) {
            if (_.isRestore) {
                ...
                self.updatePgConf(opts, function (err) {
                    _.isRestore = err;
                    return cb();
                });
            }
    ...
    function updatePrimaryConnInfo(cb) {
        ...
        self.updateRecoveryConf(opts, cb);
    }
...
PostgresMgr.prototype.getUrl = function getUrl() {
    var self = this;
    assert.object(self.url, 'this.url');
    return self.url;
};
 ...
 * @param {PostgresMgr-cb} callback
 */
PostgresMgr.prototype.stop = function (callback) {
    var self = this;
    var log = self.log;
 ...
 * @param {PostgresMgr-cb} callback
 */
PostgresMgr.prototype.startHealthCheck = function (callback) {
    var self = this;
    var log = self.log;
...
PostgresMgr.prototype.stopHealthCheck = function (callback) {
    var self = this;
    var log = self.log;
 ...
 */
PostgresMgr.prototype.start = function start(cb) {
    var self = this;
    var log = self.log;
    var msg = '';
    var intervalId = null;
 ...
 */
PostgresMgr.prototype.initDb = function (callback) {
    var self = this;
    var log = self.log;
    log.info({
        dataDir: self.dataDir
 ...
 */
PostgresMgr.prototype.deleteFile = function (path, callback) {
    var self = this;
    var log = self.log;
...
PostgresMgr.prototype.queryDb = function (queryStr, callback) {
    var self = this;
    var log = self.log;
    var calledBack = false;
    log.trace({
    ...
    if (!self.pgClient) {
        self.pgClient = new Client(self.url.href);
 ...
 */
PostgresMgr.prototype.sighup = function (callback) {
    var self = this;
    var log = self.log;
 ...
 */
PostgresMgr.prototype.updatePgConf = function (options, cb) {
    var self = this;
    self.updateConf(options, self.postgresConf, self.postgresConfPath, cb);
}
 ...
 * in the default config will be lost.
 */
PostgresMgr.prototype.updateRecoveryConf = function (options, cb) {
    var self = this;
    self.updateConf(options, self.recoveryConf, self.recoveryConfPath, cb);
}

PostgresMgr.prototype.updateConf = function (options, rpath, wpath, cb) {
    var self = this;
    var log = self.log;
    log.debug({
        options: options,
 ...
 * @param {function} callback The callback of the form f(err).
 */
PostgresMgr.prototype.restart = function (callback) {
    var self = this;
    var log = self.log;
 ...
 */
PostgresMgr.prototype.health = function (callback) {
    var self = this;
    var log = self.log;
    log.trace('Postgresman.health: entering');
    self.queryDb('select current_time;', function (err) {
        if (err) {
 ...
 */
PostgresMgr.prototype.checkRepl = function (cb) {
    var self = this;
    var log = self.log;
    var replReplayLoc = null;
    var replStartTime = Date.now();
    var intervalId = setInterval(function () {
        self.checkReplStatus(intervalId, function (err, _stop, replayLoc) {
            if (err) {
...
PostgresMgr.prototype.checkReplStatus = function (intervalId, callback) {
    var self = this;
    var log = self.log;
    ...
    self.queryDb('select  * from pg_stat_replication', function (err, result) {
...
SyncStateChecker.prototype.start = function start(primaryUrl, myUrl, cb) {
    ...
    var queryString = sprintf(SYNC_STATE_PG_QUERY, myUrl.hostname);
