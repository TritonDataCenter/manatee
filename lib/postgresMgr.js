        ...
        */
        function updatePrimaryConnInfo(cb) {
                ConfParser.read(self.recoveryConf, function(err, conf) {
                        ...
                        }, 'PostgresMgr.standby: updating primary conn info');
...
/**
 * Updates synchronous_standby_names and sets synchronous_commit=on Note, keys
 * in the current config not present in the default config will be lost
 */
...
//TODO: stop polling after sometime? what happens if the standby never catches up? we are blocked and never get interrupted. We need to make this operation interruptable.
function checkReplStatus(self, callback) {
        var log = self.log;
        ...
        query.on('row', function(row) {
                ...
                var sentLocation = row.sent_location.split('/')[1];
                sentLocation = parseInt(sentLocation, 16);
                var replayLocation = row.replay_location.split('/')[1];
                replayLocation = parseInt(replayLocation, 16);

                log.info({primary: sentLocation, standby: replayLocation}, 'xlog locations are');

                if (sentLocation === replayLocation) {
                        log.info({
                                row: row
                        }, 'exiting chekReplStatus: ' +
                        ...
                        setTimeout(function(){checkReplStatus(self, callback);},
                                   1000);
                }
        });
