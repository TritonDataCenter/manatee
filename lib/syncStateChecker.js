...
SyncStateChecker.prototype.start = function start(primaryUrl, myUrl, cb) {
    ...
    function createPgClient(pgUrl, _cb) {
         ...
         */
        self._pgClient.on('error', function (err) {
            log.info({err: err}, 'SyncStateChecker.querySyncState: PG error');
        });
    ...
    /*
     * The recursive var just prevents recursive calls of querySyncState from
     * going more than 1 function deep. We don't want persistent errors from the
     * PG client to get us stuck in an infinite recursion loop.
     */
    function querySyncState(recursive) {
        if (!self._pgClient) {
            ...
            self._pgClient.query(queryString, function (err, result) {
                if (err) {
                    ...
                    /*
                     * MANATEE-145:
                     * close the client and d/c on error so we can reconnect
                     * next time. If the current client's remote SIGHUPs, the
                     * client never gets an error or end event, which will
                     * result in the blackholing of future requests.
                     */
                    try {
                        self._pgClient.end();
                        self._pgClient = null;
                    ...
                    if (result.rows[0] && result.rows[0]['sync_state']) {
                        ...
                        fs.writeFile(self._cookieLocation,
...
SyncStateChecker.prototype.getLastRole = function (cb) {
    var self = this;
    self._log.info({
        cookieLocation: self._cookieLocation
    ...
    fs.readFile(self._cookieLocation, 'utf8', function (err, data) {
        if (err) {
            ...
            self._log.info({err: err}, 'unable to get last role');
            return cb();
        }
        var state;
        if (data) {
