...
function ZfsClient(options) {
    ...
    this.dataset = options.dataset; /* zfs dataset of this peer */
    this.parentDataset = options.parentDataset;

    this.mountpoint = options.mountpoint; /* zfs mountpoint of the dataset */
    ...
    this.zfsHost = options.zfsHost; /* ip addr used for zfs recv */

    this.zfsPort = options.zfsPort; /* port used for zfs recv */

    this.zfs_recv = options.zfsRecvPath; /* path to the zfs_recv binary */

    this.pollInterval = options.pollInterval; /* backup server poll interval*/
