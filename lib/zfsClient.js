...
function ZfsClient(options) {
    ...
    this.log = options.log.child({component: 'ZfsClient'}, true);
    self.log.info('initializing ZfsClient with options', options);
...
ZfsClient.prototype.restore = function restore(callback) {
    ...
    var tasks = [
        function _backupCurrentDataset(_, cb) {
            self.backupDataset(self.dataset, self.parentDataset + '/' + uuid(),
        ...
        function _receive(_, cb) {
            self.receive(self.dataset, self.serverUrl, self.pollInterval, cb);
        },
                                        ...
                                        callback)
{
    ...
    var tasks = [
        ...
        function _pollRestoreCompletion(_, cb) {
            self.pollRestoreCompletion(self.serverUrl,
