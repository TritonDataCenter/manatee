 ...
 * @param {string} options.parentDataset Parent ZFS dataset.
 * @param {string} options.dbUser PostgreSQL DB user.
 * @param {string} options.mountpoint Mountpoint for the ZFS dataset.
 * @param {string} [options.serverUrl] Remote restore server URL.
 ...
 * ms.
 * @param {string} options.zfsRecvPath Path of the ZFS recv binary
 */
function ZfsClient(options) {
    ...
    this._parentDataset = options.parentDataset;
    ...
    /** @type {string} Backup server's url. e.g. http://10.0.0.0:1234 */
    ...
    /** @type {string} Path to the zfs_recv binary */
    this._zfs_recv = options.zfsRecvPath;
...
ZfsClient.prototype.restore = function restore(callback) {
    ...
    var tasks = [
        function _backupCurrentDataset(_, cb) {
            self.backupDataset(self._dataset, self._parentDataset + '/' + uuid(),
                self._mountpoint, function (err, snap)
                {
                    backupSnapshot = snap;
                    return cb(err);
                });
        ...
        function _receive(_, cb) {
            self.receive(self._dataset, self._serverUrl, self._pollInterval, cb);
        },
        function _setMountPoint(_, cb) {
            var cmd = 'zfs set mountpoint=' +
                self._mountpoint + ' ' + self._dataset;
...
ZfsClient.prototype.restoreDataset = function restoreDataset(snapshot, callback)
{
    ...
    var tasks = [
        ...
        function _chownMountPoint(_, cb) {
            var cmd = 'sudo chown -R ' + self._dbUser + ' ' + self._mountpoint;
            log.info({cmd: cmd}, 'ZfsClient.restoreDataset: exec');
            exec(cmd, cb);
        },
 ...
 */
ZfsClient.prototype.backupDataset = function (dataset,
                                              backup,
                                              mountpoint,
                                              callback)
{
 ...
 */
ZfsClient.prototype.postRestoreRequest = function (callback) {
    var self = this;
    var log = self._log;
    var request = {
        host: self._zfsHost,
    ...
    self._client.post('/backup', request, function (err, req, res, obj) {
        ...
        return callback(err, obj.jobPath);
    });
}
 ...
 */
ZfsClient.prototype.pollRestoreCompletion = function (serverUrl,
                                                      pollInterval,
                                                      restoreIntervalId,
                                                      jobPath,
                                                      callback)
 ...
 * @param {ZfsClient-cb} callback
 */
ZfsClient.prototype.receive = function (dataset,
                                        serverUrl,
                                        pollInterval,
                                        callback)
{
    ...
    var tasks = [
        ...
        function _startZfsRecv(_, cb) {
            var cmd = self._zfs_recv + ' 0 ' + self._zfsPort +
                ' -u ' + self._dataset;
            log.info({cmd: cmd}, 'ZfsClient.receive: exec');
            exec(cmd, function (err) {
                clearInterval(restoreIntervalId);
        ...
        function _postRestoreRequest(_, cb) {
            self.postRestoreRequest(function (err, jobPath) {
                _.jobPath = jobPath;
                return cb(err);
            });
        },
        ...
        function _pollRestoreCompletion(_, cb) {
            self.pollRestoreCompletion(self._serverUrl,
                self._pollInterval, restoreIntervalId,
            _.jobPath, cb);
        }
    ];
