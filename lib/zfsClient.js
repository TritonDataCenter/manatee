...
ZfsClient.prototype.assertDataset = function assertDataset(callback) {
    ...
    var tasks = [
        ...
        function _createDataset(_, cb) {
            ...
            var cmd = 'zfs create ' + self.dataset;
            log.debug({
                cmd: cmd
            }, 'ZfsClient.assertDataset: manatee dataset DNE creating dataset');
            exec(cmd, cb);
...
function receive(self, dataset, serverUrl, pollInterval, callback) {
    ...
    var tasks = [
        ...
        // MANATEE-94
        // destroyCurrentDataset() should already have removed everything under
        // the mountpoint, however, to be safe, we rmr $mountpoint/*
        function rmrMountpoint(_, cb) {
            var cmd = 'pfexec rm -rf ' + self.mountpoint + '/*';
            log.info({cmd: cmd}, 'ZfsClient.receive: exec');
            exec(cmd, cb);
        },
