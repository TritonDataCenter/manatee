...
ZfsClient.prototype.restore = function restore(callback) {
    ...
    var tasks = [
        function _backupCurrentDataset(_, cb) {
            self._backupDataset(self._dataset, self._parentDataset + '/' + uuid(),
        ...
        function _receive(_, cb) {
            self._receive(self._dataset, self._serverUrl, self._pollInterval, cb);
        },
 ...
 */
ZfsClient.prototype._postRestoreRequest = function (callback) {
    var self = this;
    var log = self._log;
    var request = {
        host: self._zfsHost,
                                        ...
                                        callback)
{
    ...
    var tasks = [
        ...
        function _postRestoreRequest(_, cb) {
            self._postRestoreRequest(function (err, jobPath) {
                _.jobPath = jobPath;
                return cb(err);
            });
        },
        ...
        function _pollRestoreCompletion(_, cb) {
            self._pollRestoreCompletion(self._serverUrl,
