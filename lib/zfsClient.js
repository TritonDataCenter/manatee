...
function ZfsClient(options) {
        ...
        */
        ...
        /**
         * intervalId used for polling restore tasks
         */
        ...
        var tasks = [
                ...
                function _startZfsRecv(_, cb) {
                        ...
                        spawn(cmd, log, function(err) {
                                clearInterval(self.restoreIntervalId);
                                log.info({
                                        err: err,
                                        dataset: self.dataset,
                                        backupDataset: backupDataset,
                                        ...
                                        serverUrl: self.serverUrl,
                                        pollInterval: self.pollInterval,
                                        restoreIntervalId: self.restoreIntervalId
                                }, 'ZfsClient.receive: finished');
                                self.restoreIntervalId = null;

                                return callback(err, backupDataset);
                        });
                        return cb();
                },
                // then start the send
...
function pollRestoreCompletion(self, jobPath, callback) {
        ...
        callback = once(callback);
        self.restoreIntervalId = setInterval(function() {
                // clearInterval() may not stop any already enqueued tasks, so
                // we only return callback if restoreIntervalId is not null
                if (!self.restoreIntervalId) {
                ...
                log.debug({
                        ...
                        pollInterval: self.pollInterval,
                        jobPath: jobPath
                }, 'getting restore job status', self.restoreIntervalId);
                ...
                self.client.get(jobPath, function(err, req, res, obj) {
                        ...
                        if (obj.done === true) {
                                ...
                                log.info({
                                        ...
                                        jobPath: jobPath,
                                        pollInterval: self.pollInterval
                                }, 'restore job failed', self.restoreIntervalId);
                                ...
                                log.info({
                                        ...
                                        jobPath: jobPath,
                                        pollInterval: self.pollInterval
                                }, 'restore job not complete', self.restoreIntervalId);
                                return true;
                        }
                });
        }, self.pollInterval);
