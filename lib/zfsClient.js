...
ZfsClient.prototype.assertDataset = function assertDataset(callback) {
    ...
    var tasks = [
        function checkDatasetExists(_, cb) {
        ...
        function isDatasetMounted(_, cb) {
            var cmd = 'zfs get -Hp mounted ' + self.dataset + ' | cut -f2';
            log.debug({
                cmd: cmd
