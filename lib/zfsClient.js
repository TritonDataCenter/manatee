 ...
 */
ZfsClient.prototype.restoreDataset = function restoreDataset(snapshot, cb) {
        var self = this;
        var log = self.log;
        ...
        var tasks = [
                function _destroyDataset(_, cb) {
                        spawn('pfexec zfs destroy -r ' + dataset, log,
                                function(err) {
                                        log.info({err:
                                                new verror.VError(
