 ...
 * ms.
 * @param {string} options.zfsPath Path of the ZFS recv binary
 *
                                        ...
                                        callback)
{
    ...
    vasync.pipeline({funcs: [
        ...
        function _startZfsRecv(_, cb) {
             ...
             *  the mountpoint
             */
            _.zfsRecv = spawn(self._zfsPath, ['recv', '-u', self._dataset]);
            ...
            _.zfsRecv.on('exit', function (code) {
                if (code !== 0) {
                    ...
                    log.error({err: err2},
                              'zfsClient._receive: unable to complete zfs recv');
                    cb(err2);
                }
        ...
        function createServer(_, cb) {
            ...
            log.info({port: self._zfsPort, host: 'localhost'},
                     'listening for zfs send on');
            server.listen(self._zfsPort, 'localhost', 1, function (err) {
                if (err) {
                    log.error({err: err},
            ...
            server.on('error', function (err) {
                log.error({err: err}, 'ZfsClient._receive: got socket error');
        ...
        function _pollRestoreCompletion(_, cb) {
            self._pollRestoreCompletion(self._serverUrl, self._pollInterval,
