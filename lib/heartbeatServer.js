...
HeartbeatServer.prototype.init = function init() {
    ...
    function postHeartbeat(req, res, next) {
        ...
        if (!params.postgresUrl) {
            return next(new restify.MissingParameterError(
                'postgresUrl parameter required'));
        }

        if (!self._heartbeat[params.postgresUrl]) {
            self._heartbeat = {};
            self._heartbeat[params.postgresUrl] = Date.now();
            log.info({
             ...
             * @type {string} the postgres URL of the standby.
             */
            self.emit('newStandby', params.postgresUrl);
        } else {
            log.trace({
                heartbeat: self._heartbeat,
                params: params,
...
HeartbeatServer.prototype._checkHeartbeats = function () {
    ...
    for (var host in self._heartbeat) {
        var hostTime = parseInt(self._heartbeat[host], 10);
        if (currTime - hostTime >= self._expirationTime) {
            log.info({
                host: host,
                hostTime: hostTime,
                currTime: currTime,
