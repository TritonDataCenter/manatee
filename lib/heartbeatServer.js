...
function HeartbeatServer(options) {
     ...
     */
    this._heartbeatPollInterval = options.heartbeatPollInterval;
...
HeartbeatServer.prototype.init = function init() {
    ...
    function postHeartbeat(req, res, next) {
        ...
        if (!self._heartbeat[params.host]) {
            self._heartbeat = {};
            self._heartbeat[params.host] = Date.now();
            log.info({
                host: params.host,
             ...
             */
            self.emit('newStandby', params.host + ':' + params.pgPort);
        } else {
            log.trace({
                host: params.host,
                heartbeat: self._heartbeat,
...
HeartbeatServer.prototype._checkHeartbeats = function () {
    ...
    for (var host in self._heartbeat) {
        ...
        if (currTime - hostTime >= self._expirationTime.gc) {
            log.info({
                host: host,
                hostTime: hostTime,
                currTime: currTime,
