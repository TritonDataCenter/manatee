...
Daemon.prototype.primary = function primary(callback) {
   ...
   * 0) Delete recovery.conf
   * 1) Try and initialize the db.
   * 2) Create the repl_tracking table.
   * 3) Start repl_tracking update daemon.
    ...
    self.checkErr(err, function(){
      ...
      self.deleteFile(self.recoveryPath, function(err) {
        ...
        if (!self.shard.sync && self.readOnly) {
          ...
          return updateStandbySetSyncCommit(function(err) {
            self.checkErr(err, function() {
              return self.postgresMan.restart(callback);
            });
          });
      ...
      var updateStandbySetSyncCommit =
        function updateStandbySetSyncCommit(callback)
      {
...
/**
 * Check how far behind the primary the standby peers are. If the standby falls
 * behind past a certain threshold, restart the standby.
 */
Daemon.prototype.replStatusCheck = function replStatusCheck() {

};

/**
 * Update the primary's xlog position. Note this only runs if the current Peer
 * is the primary.
 */
Daemon.prototype.updateXlog = function updateXlog(callback) {
  var self = this;
  var log = self.log;
  log.debug('entering update xlog');

  if (self.mode === MODE.PRIMARY) {
    // first get the xlog
    self.postgresMan.xlogLocation(function(err, xlog) {
      self.checkErr(err, function() {
        return self.shard.writeXlog(xlog, callback);
      });
    });
  }
};
